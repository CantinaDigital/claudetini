...
def dispatch_task(
    ...
    """Run Codex CLI and return normalized dispatch output.
    ...
    if output_file is None:
        output_file = dispatch_output_dir / f"{session_id}-codex.log"
    # Codex CLI: use 'exec' subcommand for non-interactive execution
    command = [cli_path, "exec", prompt]
    ...
    try:
        ...
        return DispatchResult(
            success=False,
            session_id=session_id,
            error_message=f"Codex CLI not found at '{cli_path}'.",
            output_file=output_file,
            provider="codex",
        )
            ...
            while True:
                ...
                if (
                    ...
                    return DispatchResult(
                        ...
                        session_id=session_id,
                        error_message=(
                            f"Codex CLI stalled with no output for {stall_timeout_seconds}s."
                        ),
                        output_file=output_file,
                        output=output or None,
                        provider="codex",
    ...
    if proc.returncode != 0:
        return DispatchResult(
            success=False,
            session_id=session_id,
            error_message=_extract_error_message(output, proc.returncode, "Codex CLI"),
            output_file=output_file,
            output=output or None,
            provider="codex",
        )
